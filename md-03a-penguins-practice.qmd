...
5.  What data types are contained in the variables? (Reminder: <https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables>) Categorical, continuous and discrete
...
Starting with `penguins`, group the data by species, then create a summary table containing the maximum and minimum length of flippers (call the columns flip_max and flip_min). How will you handle NA values? **I will remove NAs before the summary of the variable since it is just a single variable**
...
Starting with `penguins`, group the data by species and year, then create a summary table containing the mean bill depth (call this bill_depth_mean), the mean bill length (call this bill_length_mean), and the count for each group. How will you handle NA values? **I will ignore NAs in the calculation for each of the variables since there are multiple variables**
...
penguins |> 
  ...
  summarise(bill_depth_mean = mean(bill_depth_mm, na.rm = TRUE),
            bill_length_mean = mean(bill_length_mm, na.rm = TRUE), n = n())
